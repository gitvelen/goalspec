#!/usr/bin/env bash
# GOALC #8: memory-patch.yaml change makes old memory-patch approval stale;
#            without re-approval complete must fail.
...
patches:
  - kind: capability
    content:
      ...
      status: active
YML
"$REPO_GS" approve memory-patch >/dev/null && ok "memory-patch approved"
# now change memory-patch.yaml -> approval should be stale
yq e -i '.patches[0].content.statement = "TAMPERED"' "$REPO/.goalspec/active/memory-patch.yaml"
if "$REPO_GS" complete >/dev/null 2>&1; then
  bad "complete succeeded with stale memory-patch approval"
else
  ok "complete blocked with stale memory-patch approval"
fi
