...
assert_managed_ai_guide() {
  ...
  /bin/grep -q '范围与优先' "$file" || bad "$file missing scope/priority section"
  /bin/grep -q '人类命令映射' "$file" || bad "$file missing command map section"
  /bin/grep -q 'Criteria 审查最低要求' "$file" || bad "$file missing criteria review minimum"
