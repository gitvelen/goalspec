#!/usr/bin/env bash
# GOALC #12: without fresh guardian verdict, even with exit code 0 / evidence,
#            complete must fail.
. "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/lib.sh"

fresh_initialized_repo goalc-12
"$REPO_GS" new-goal "test" >/dev/null
make_minimal_goal_md "$REPO/.goalspec/active/goal.md"
approve_intake_and_goal
"$REPO_GS" compile >/dev/null
make_minimal_contract "$REPO/.goalspec/active/contract.yaml"
tmp="$TESTS_TMP_ROOT/p12"; mkdir -p "$tmp"
cat > "$tmp/c.yaml" <<'YML'
kind: contract
result: pass
blocking_questions: []
...
# Executor writes evidence with exit_code 0 and a self-claim of "done" but no verdict.
...
# memory-patch present (so we know it's the missing verdict that blocks complete, not mpatch).
...
if "$REPO_GS" complete >/dev/null 2>&1; then
  bad "complete succeeded with no guardian verdict"
else
  ok "complete blocked without guardian verdict"
fi
