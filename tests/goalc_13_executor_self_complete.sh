#!/usr/bin/env bash
# GOALC #13: Subagent's evidence/trace self-claim "done" / "passed" must not let
#            state enter completed; only Master verdict + complete can.
...
if [ "$status" = "completed" ]; then
  bad "state became completed from Subagent self-claim"
else
  ok "state not completed from Subagent self-claim (still $status)"
fi
