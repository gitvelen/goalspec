...
if echo "$status_out" | /bin/grep -q 'CURRENT_WORK_UNIT'; then
...
for k in state goal frozen prompt_ready run_allowed needs_human_confirmation blockers unmet_criteria next_user_action; do
